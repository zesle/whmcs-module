...
class Pagination {
    ...
    function createLinks( $culimit ) {
        ...
        if ($this->currentPage > $this->numLinks) {
            $output .= $this->firstTagOpen
                    . $this->getAJAXlink('', $this->firstLink, $culimit )
                    . $this->firstTagClose;
        }
        ...
        if ($this->currentPage != 1) {
            ...
            $output .= $this->prevTagOpen
                    . $this->getAJAXlink($i, $this->prevLink, $culimit)
                    . $this->prevTagClose;
        }
            ...
            if ($i >= 0) {
                if ($this->currentPage == $loop) {
                    ...
                    $output .= $this->numTagOpen
                            . $this->getAJAXlink($n, $loop, $culimit )
                            . $this->numTagClose;
                }
            }
        }
        ...
        if ($this->currentPage < $numPages) {
            $output .= $this->nextTagOpen
                    . $this->getAJAXlink($this->currentPage * $this->perPage, $this->nextLink, $culimit )
                    . $this->nextTagClose;
        }
        ...
        if (($this->currentPage + $this->numLinks) < $numPages) {
            ...
            $output .= $this->lastTagOpen . $this->getAJAXlink($i, $this->lastLink, $culimit ) . $this->lastTagClose;
        }
    ...
    function getAJAXlink($count, $text, $culimit) {
        if ($this->contentDiv == '')
            return '<a href="' . $this->anchorClass . ' ' . $this->baseURL . $count . '">' . $text . '</a>';

        $pageCount = $count ? $count : 0;
        $this->additionalParam = "{'page' : $pageCount, 'limit': $culimit }";
